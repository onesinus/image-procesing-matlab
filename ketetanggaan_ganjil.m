function [G] = ketetanggaan_ganjil(fn)
    F = fn;
    Ukuran = size(F);
    tinggi = Ukuran(1);
    lebar = Ukuran(2);
    ...
    for baris = 2 : tinggi-1
        for kolom = 2 : lebar-1
            % Mengambil nilai piksel tetangga ganjil
            tetangga = [F(baris-1, kolom-1) F(baris-1, kolom+1) ...
                        F(baris, kolom-1)     F(baris, kolom+1)   ...
                        F(baris+1, kolom-1) F(baris+1, kolom+1)];
